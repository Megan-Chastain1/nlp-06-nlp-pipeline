...
def run_transform(
    ...
    # Top token
    top_token = Counter(tokens).most_common(1)
    LOG.info(f"  top_token: {top_token}")

    # Top token definition using WordNet
    top_definition = "unknown"
    for word, _count in top_token:
        # Get WordNet data
        synsets = wordnet.synsets(word)

        if synsets:
            # Grab the top definition
            top_definition = synsets[0].definition()
    LOG.info(f"  top_definition: {top_definition}")
